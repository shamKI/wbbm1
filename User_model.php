...
class User_model extends CI_Model{
    ...
    public function get_users($fields = '', $criteria = '', $offset = '', $limit = '', $order_by = '', $join1 = '', $group_by = '', $join2 = '', $join3 = '', $criteria_in = '') {
        ...
        if ($order_by != '') { 
            ...
            $this->db->order_by($this->table . '.user_status DESC, ' . $this->table . '.first_name ASC,' . $this->table . '.last_name ASC'); 
        }
    ...
    public function get_user($fields = '', $criteria = '', $join1 = '', $join2 = '') {
        if ($fields != '') { $this->db->select($fields); }
        if ($criteria != '') { $this->db->where($criteria); }
        if ($join1 != '') { $this->db->join($join1[0], $join1[1], 'left'); }
        if ($join2 != '') { $this->db->join($join2[0], $join2[1], 'left'); }
    ...
    public function update_user($data_set, $criteria) {
        return $this->db->update($this->table,$data_set, $criteria);
    }//end of function

    public function delete_user($criteria) {
