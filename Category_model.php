...
class Category_model extends CI_Model{
    ...
    public function get_categories($fields = '', $criteria = '', $offset = '', $limit = '', $order_by = '', $join1 = '', $group_by = '', $join2 = '', $join3 = '', $criteria_in = '') {
        ...
        if ($order_by != '') { 
            ...
            $this->db->order_by($this->table . '.category_status DESC, ' . $this->table . '.category_name ASC'); 
        }
    ...
    public function get_category($fields = '', $criteria = '', $join1 = '', $join2 = '') {
        if ($fields != '') { $this->db->select($fields); }
        if ($criteria != '') { $this->db->where($criteria); }
        if ($join1 != '') { $this->db->join($join1[0], $join1[1], 'left'); }
        if ($join2 != '') { $this->db->join($join2[0], $join2[1], 'left'); }
    ...
    public function add_category($data_set) {
        return $this->db->insert($this->table, $data_set);
    }//end of function

    public function update_category($data_set, $criteria) {
        return $this->db->update($this->table,$data_set, $criteria);
    }//end of function

    public function delete_category($criteria) {
