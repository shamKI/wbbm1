...
class Sales_book_model extends CI_Model{
	...
	private $table = 'tbl08_sales_book';
    ...
    public function get_sales_books($fields = '', $criteria = '', $offset = '', $limit = '', $order_by = '', $join1 = '', $group_by = '', $join2 = '', $join3 = '', $criteria_in = '') {
        ...
        if ($order_by != '') { 
            ...
            $this->db->order_by($this->table . '.sales_sku ASC'); 
        }
    ...
    public function get_sales_book($fields = '', $criteria = '', $join1 = '', $join2 = '') {
        if ($fields != '') { $this->db->select($fields); }
        if ($criteria != '') { $this->db->where($criteria); }
        if ($join1 != '') { $this->db->join($join1[0], $join1[1], 'left'); }
        if ($join2 != '') { $this->db->join($join2[0], $join2[1], 'left'); }
    ...
    public function add_sales_books($data_set){
        return $this->db->insert_batch($this->table,$data_set);
    }//end of function

    public function update_sales_book($data_set, $criteria) {
        return $this->db->update($this->table,$data_set, $criteria);
    }//end of function

    public function delete_sales_book($criteria) {
        return $this->db->delete($this->table, $criteria);
    }//end of function
    
    public function update_sales_books($data_set,$criteria){
