...
class Dealer_model extends CI_Model{
    ...
    public function get_dealers($fields = '', $criteria = '', $offset = '', $limit = '', $order_by = '', $join1 = '', $group_by = '', $join2 = '', $join3 = '', $criteria_in = '') {
        ...
        if ($order_by != '') { 
            ...
            $this->db->order_by($this->table . '.dealer_status DESC, ' . $this->table . '.dealer_name ASC'); 
        }
    ...
    public function get_dealer($fields = '', $criteria = '', $join1 = '', $join2 = '') {
        if ($fields != '') { $this->db->select($fields); }
        if ($criteria != '') { $this->db->where($criteria); }
        if ($join1 != '') { $this->db->join($join1[0], $join1[1], 'left'); }
        if ($join2 != '') { $this->db->join($join2[0], $join2[1], 'left'); }
    ...
    public function add_dealer($data_set) {
        return $this->db->insert($this->table, $data_set);
    }//end of function

    public function update_dealer($data_set, $criteria) {
        return $this->db->update($this->table,$data_set, $criteria);
    }//end of function

    public function delete_dealer($criteria) {
