...
class Sales_return_model extends CI_Model{
	...
	private $table = 'tbl09_sales_return';
    ...
    public function get_sales_returns($fields = '', $criteria = '', $offset = '', $limit = '', $order_by = '', $join1 = '', $group_by = '', $join2 = '', $join3 = '', $criteria_in = '') {
    ...
    public function get_sales_return($fields = '', $criteria = '', $join1 = '', $join2 = '') {
        if ($fields != '') { $this->db->select($fields); }
        if ($criteria != '') { $this->db->where($criteria); }
        if ($join1 != '') { $this->db->join($join1[0], $join1[1], 'left'); }
        if ($join2 != '') { $this->db->join($join2[0], $join2[1], 'left'); }
    ...
    public function add_sales_returns($data_set){
        return $this->db->insert_batch($this->table,$data_set);
    }//end of function

    public function update_sales_return($data_set, $criteria) {
        return $this->db->update($this->table,$data_set, $criteria);
    }//end of function
    
    public function update_sales_returns($data_set, $criteria) {
        return $this->db->update_batch($this->table,$data_set,$criteria);
    }//end of function

    public function delete_sales_return($criteria) {
